...
var slashCommands = map[string]commandFunc{
	"/changelog": appModel.cmdChangelog,
...
func (m appModel) cmdChangelog(text string, fields []string) (appModel, tea.Cmd) {
	if m.conv == nil || m.user == nil {
		return m, nil
	}
	if len(fields) > 2 {
		return m.sys("usage: /changelog [version]"), nil
	}
	m = m.echo(text)
...
func (m appModel) cmdAdmin(text string, fields []string) (appModel, tea.Cmd) {
	...
	case "update":
		m2, _, cmd := m.handleAdminUpdateCommand(text, fields)
		return m2, cmd
	}
	return m.sys(adminUsageBlock()), nil
}
...
func (m appModel) cmdTools(text string, fields []string) (appModel, tea.Cmd) {
	...
	} else if fields[1] == "categories" {
		return m.sys(chatcmd.ToolsCategories(m.toolReg)), nil
